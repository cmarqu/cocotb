...
class CustomLogFormatter(SimColourLogFormatter):
    ...
    * Prints a vertical "bracket" highlighting timestamp changes:
    ...
    def format_sim_time(self, record, time_chars=12, time_base="us"):
...
async def test_custom_logger(dut):
    ...
    # print("format_recordname:", fmt.format_recordname.cache_info())
    # print("format_filename:", fmt.format_filename.cache_info())
    # print("format_funcname:", fmt.format_funcname.cache_info())
    assert (
        fmt.format_recordname.cache_info().hits > 0
    ), "Nothing was taken from the format_recordname LRU cache"
    assert (
        fmt.format_filename.cache_info().hits > 0
    ), "Nothing was taken from the format_filename LRU cache"
    assert (
        fmt.format_funcname.cache_info().hits > 0
    ), "Nothing was taken from the format_funcname LRU cache"
