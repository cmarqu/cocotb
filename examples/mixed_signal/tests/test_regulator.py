...
class Regulator_TB(object):
    ...
    def __init__(self, dut, settling_time_ns=1):
        ...
        self._bittogglestream = itertools.cycle(
            [0, 1]
        )  #: Produce stream that toggles between ``0`` and ``1``.
    ...
    @cocotb.coroutine
    def find_trim_val(
        self, probed_node, target_volt, trim_val_node, trim_val_repr="TWOS_COMPLEMENT"
    ):
    ...
    def plot_data(self, datasets, graphfile="cocotb_plot.png"):
        ...
        color = "tab:red"
        ax1.set_title("Probed node: {}".format(self.analog_probe.node_to_probe))
        ax1.set_xlabel("trim")
        ax1.set_ylabel("Voltage (V)", color=color)
        ax1.plot(
            trim, voltage, color=color, marker=".", markerfacecolor="black", linewidth=1
        )
        ...
        ax2.plot(
            trim, current, color=color, marker=".", markerfacecolor="black", linewidth=1
        )
...
def run_test(dut):
    ...
    target_volt = 3.013
    print(
        "test_regulator.py: Running trimming algorithm for target voltage {:.3} V".format(
            target_volt
        )
    )
    ...
    print(
        ...
        "which gives a trimmed voltage of {:.3} V (difference to target {:.3} V)".format(
