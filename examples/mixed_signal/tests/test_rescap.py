...
class ResCap_TB(object):
    ...
    def probe_values(self):
        ...
        self.tb_hdl._log.info(
            "{}({})={} V  {} A".format(
                self.analog_probe, self.analog_probe.node_to_probe, voltage, current
            )
        )
        return Dataset(time=get_sim_time(units="ns"), voltage=voltage, current=current)
    ...
    def _get_single_sample(self, node):
        ...
        self.tb_hdl._log.debug(
            "{}={} V, {} A".format(
                self.analog_probe.node_to_probe, dataset.voltage, dataset.current
            )
        )
...
#        return datasets
    ...
    def plot_data(self, datasets, nodes, graphfile="cocotb_plot.png"):
        ...
        color = "tab:red"
        ax1.set_title("Probed nodes: {}".format(nodes))
        ax1.set_xlabel("Time (ns)")
        ax1.set_ylabel("Voltage (V)", color=color)
        ax1.plot(
            time, voltage, color=color, marker=".", markerfacecolor="black", linewidth=1
        )
        ...
        ax2.plot(
            time, current, color=color, marker=".", markerfacecolor="black", linewidth=1
        )
...
def run_test(tb_hdl):
    ...
    dummy = yield tb_py.get_sample_data(
        nodes=nodes_to_probe,
        num=2,  # FIXME: does not work with num=1
    ...
    data = yield tb_py.get_sample_data(num=80, delay_ns=5, nodes=nodes_to_probe)
