...
def test_trim(hdl_tb):
    ...
    hdl_tb.vss_val <= 0.0
    print(
        "Running automatic trimming algorithm for target voltage {} V".format(
            target_volt
        )
    )
    best_trim = yield find_trim_val(
        hdl_tb, probed_node, target_volt, trim_val_node=hdl_tb.trim_val
    )
    print("Best trimming value is {}".format(best_trim))
    # measure back
    hdl_tb.trim_val <= best_trim
    yield Timer(1, units="ns")
    trimmed_volt = yield get_single_sample(hdl_tb, probed_node)
    print(
        "Trimmed voltage is {} V (difference to target is {} V)".format(
            trimmed_volt, trimmed_volt - target_volt
        )
    )
