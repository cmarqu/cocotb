...
The testcase supplies the circuit with ``vdd`` and calls the trimming routine 
:meth:`~test_regulator.Regulator_TB.find_trim_val()`
...
are printed.
The output voltage will likely not exactly hit the target value because of 
...
It then runs an automatic trimming routine :meth:`~test_regulator.Regulator_TB.find_trim_val()` 
