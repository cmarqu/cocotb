...
in series in a SystemVerilog module, as shown here:
...
.. note:: This analog probe module is currently only implemented for the 
          Cadence Incisive and Xcelium simulators.
...
The testcase supplies ``vdd`` and measures three pairs of voltage and current at the 
``p`` terminal of the capacitor, spaced 50 ns apart.
...
The sampled voltages and currents continually switch between the node where the supply is 
(``tb_rescap.i_rescap.vdd``), and the output voltage ``tb_rescap.i_rescap.vout``.
