...
def test_init_little_endian_unsigned():
    ...
    bin6 = BinaryValue(value="11111111111111111111110000101100", n_bits=32,
                       bigEndian=False, binaryRepresentation=BinaryRepresentation.UNSIGNED)
...
def test_init_little_endian_twos_comp():
    bin1 = BinaryValue(value=3, n_bits=4, bigEndian=False, binaryRepresentation=BinaryRepresentation.TWOS_COMPLEMENT)
    assert bin1._str == "0011"
    assert bin1.binstr == "0011"
    assert bin1.integer == 3

    bin2 = BinaryValue(value=-1, n_bits=2, bigEndian=False, binaryRepresentation=BinaryRepresentation.TWOS_COMPLEMENT)
    assert bin2._str == "11"
    assert bin2.binstr == "11"
    assert bin2.integer == -1

    bin3 = BinaryValue(value=-65, n_bits=8, bigEndian=False, binaryRepresentation=BinaryRepresentation.TWOS_COMPLEMENT)
    assert bin3._str == "10111111"
    assert bin3.binstr == "10111111"
    assert bin3.integer == -65

    bin4 = BinaryValue(value=1, n_bits=2, bigEndian=False, binaryRepresentation=BinaryRepresentation.TWOS_COMPLEMENT)
    assert bin4._str == "01"
    assert bin4.binstr == "01"
    assert bin4.integer == 1

    bin5 = BinaryValue(value="1001011", bigEndian=False, binaryRepresentation=BinaryRepresentation.TWOS_COMPLEMENT)
    assert bin5._str == "1001011"
    assert bin5.binstr == "1001011"
...
def test_init_short_binstr_value():
    bin1 = BinaryValue(value="0", n_bits=4, bigEndian=True, binaryRepresentation=BinaryRepresentation.UNSIGNED)
    assert bin1._str == "0000"
    assert bin1.binstr == "0000"
    assert bin1.integer == 0

    bin2 = BinaryValue(value="Z", n_bits=8, bigEndian=False, binaryRepresentation=BinaryRepresentation.UNSIGNED)
    ...
    bin3 = BinaryValue(value="01", n_bits=8,
                       bigEndian=False, binaryRepresentation=BinaryRepresentation.SIGNED_MAGNITUDE)
...
def test_index():
    ...
    bin3 = BinaryValue(value=154, n_bits=14,
                       bigEndian=True, binaryRepresentation=BinaryRepresentation.UNSIGNED)
